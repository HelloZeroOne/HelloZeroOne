% 读取轨迹配置XML文件
function [obj] = Trajectory_config_XML_read(obj)
%   1.输入参数：
...
for index1 = 1 :  obj_number
    ...
    % 读取XML文件
    doc_node = xmlread(obj(index1).trajectory_config_XML_file_path);
    ...
    if isempty(properties_array)
        ...
        for index2 = 1 : length(properties_array)
            ...
            % 获取属性名称对应节点
            property_node = Trajectory_config_node.getElementsByTagName(...
            ...
            if 1 < property_content_number
                ...
                switch class(obj(index1).(properties_array{index2}))
                    ...
                    otherwise
                        warning('REFERENCETRAJECTORYMANAGER类的对象中出现了其他未预设数据类型的属性');
                end
                
            else
                
                % 获取属性名称对应节点存储的文本内容
                stored_content = Trajectory_config_node.getElementsByTagName(...
