% 写入轨迹配置XML文件
function [] = Trajectory_config_XML_write(obj)
%   1.输入参数：
...
XML_file_version = '0.0.2'; % XML配置文件版本

%% 创建XML文档节点
doc_node = matlab.io.xml.dom.Document('Trajectory_config');
doc_node.XMLStandalone = true;
Trajectory_config_node = doc_node.getDocumentElement;

% 设置文件属性
Trajectory_config_node.setAttribute('author', XML_file_author);
Trajectory_config_node.setAttribute('version', XML_file_version);
...
if isempty(properties_array)
    warning('REFERENCETRAJECTORYMANAGER类的对象中无属性');
    return;
else
    for index1 = 1 : length(properties_array)
        ...
        % 将子节点关联到上一级节点之下
        Trajectory_config_node.appendChild(child_node);
    end
end
...
writeToFile(writer, doc_node, obj.trajectory_config_XML_file_path);
