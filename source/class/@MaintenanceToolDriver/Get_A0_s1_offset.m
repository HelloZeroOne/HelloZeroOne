% 获取第一阶段A0偏置值
function A0_s1_offset = Get_A0_s1_offset(obj)
%   1.输入参数：
%       (1)obj             MAINTENANCETOOLDRIVER类的一个实例
%   2.输出参数：
%       (1)A0_s1_offset    第一阶段A0偏置值，实数，单位：m

% 尝试运行，如果失败则弹出提示框，便于在其他电脑上独立运行时排查问题
try
    A0_s1_offset = Local_function_get_A0_s1_offset(obj.telnet_object);
catch
    uialert(obj.UI_figure, '获取第一阶段A0偏置值时出现异常.', '错误提示');
end

end

% ----- Local function GET_A0_S1_OFFSET -----
function result = Local_function_get_A0_s1_offset(tcpip_object)
%   1.输入参数：
%       (1)tcpip_object    TCP/IP连接对象
%   2.输出参数：
%       (1)result          第一阶段A0偏置值，实数，单位：m
