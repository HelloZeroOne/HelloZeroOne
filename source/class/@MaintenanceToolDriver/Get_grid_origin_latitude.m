% 获取相对坐标原点纬度数值
function grid_origin_latitude = Get_grid_origin_latitude(obj)
%   1.输入参数：
%       (1)obj                     MAINTENANCETOOLDRIVER类的一个实例
%   2.输出参数：
%       (1)grid_origin_latitude    相对坐标原点纬度数值，实数，单位：deg

% 尝试运行，如果失败则弹出提示框，便于在其他电脑上独立运行时排查问题
try
    grid_origin_latitude = Local_function_get_grid_origin_latitude(obj.telnet_object);
catch
    uialert(obj.UI_figure, '获取相对坐标原点纬度数值时出现异常.', '错误提示');
end

end

% ----- Local function GET_GRID_ORIGIN_LATITUDE -----
function result = Local_function_get_grid_origin_latitude(tcpip_object)
%   1.输入参数：
%       (1)tcpip_object    TCP/IP连接对象
%   2.输出参数：
%       (1)result          相对坐标原点纬度数值，实数，单位：deg
