...
function [obj] = Digital_output_variable_list_table2XML_convert(obj)
%   1.输入参数：
...
%% 数据有效性检查
% 数据源表格为空时报警并返回
if isempty(obj.sorted_digital_output_variable_list)
    warning('数字量输出变量清单为空！');
    return;
end
...
% 遍历配置表格
for index = 1 : height(obj.sorted_digital_output_variable_list)
    
    % 获取变量名对应的port_data节点
    port_data_node = port_data_list_node.getElementsByTagName(...
        obj.sorted_digital_output_variable_list.('variable_name'){index}).item(0);
    
    % 获取port_data节点的指定方向标签节点
    direction_tag_node = port_data_node.getElementsByTagName('destination_list').item(0);

    % 新建当前表格条目对应的节点并关联为对应指定方向标签节点的子节点
    temp_node = obj.XML_document.createElement(...
        obj.sorted_digital_output_variable_list.('DIO_name'){index});
    ...
    channel_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_digital_output_variable_list.('channel')(index)))));
    ...
    function_valid_value_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_digital_output_variable_list.('function_valid_value')(index)))));
