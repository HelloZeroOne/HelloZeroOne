...
function [obj] = Receive_TRDP_process_data_list_table2XML_convert(obj)
%   1.输入参数：
...
list_tag_node = obj.XML_document.createElement('receive_TRDP_process_data_list');
obj.XML_root_node.appendChild(list_tag_node);

% 数据源表格为空时报警并返回
if isempty(obj.sorted_receive_TRDP_process_data_list)
    warning('接收TRDP过程数据清单为空！');
    return;
end

% 遍历配置表格
for index = 1 : height(obj.sorted_receive_TRDP_process_data_list)
    
    % 新建当前表格条目对应的节点并关联为指定清单标签节点的子节点
    temp_node = obj.XML_document.createElement(...
        obj.sorted_receive_TRDP_process_data_list.('TRDP_process_data_name'){index});
    ...
    COM_ID_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_receive_TRDP_process_data_list.('COM_ID')(index)))));
    DLC_node = obj.XML_document.createElement('length');
    DLC_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_receive_TRDP_process_data_list.('byte_length')(index)))));
    enable_record_node = obj.XML_document.createElement('enable_record');
    if false == obj.sorted_receive_TRDP_process_data_list.('record_enable')(index)
