...
function [obj] = Digital_output_list_table2XML_convert(obj)
%   1.输入参数：
...
list_tag_node = obj.XML_document.createElement('digital_output_list');
obj.XML_root_node.appendChild(list_tag_node);

% 数据源表格为空时报警并返回
if isempty(obj.sorted_digital_output_list)
    warning('数字量输出清单为空！');
    return;
end

% 遍历配置表格
for index = 1 : height(obj.sorted_digital_output_list)
    
    % 新建当前表格条目对应的节点并关联为指定清单标签节点的子节点
    temp_node = obj.XML_document.createElement(...
        obj.sorted_digital_output_list.('DIO_name'){index});
    ...
    slot_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_digital_output_list.('slot')(index)))));
