...
function [obj] = Receive_CAN_message_list_table2XML_convert(obj)
%   1.输入参数：
...
list_tag_node = obj.XML_document.createElement('receive_CAN_message_list');
obj.XML_root_node.appendChild(list_tag_node);

% 数据源表格为空时报警并返回
if isempty(obj.sorted_receive_CAN_message_list)
    warning('接收CAN消息清单为空！');
    return;
end

% 遍历配置表格
for index = 1 : height(obj.sorted_receive_CAN_message_list)
    
    % 新建当前表格条目对应的节点并关联为指定清单标签节点的子节点
    temp_node = obj.XML_document.createElement(...
        obj.sorted_receive_CAN_message_list.('CAN_message_name'){index});
    ...
    record_start_byte_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_receive_CAN_message_list.('record_start_byte')(index)))));
