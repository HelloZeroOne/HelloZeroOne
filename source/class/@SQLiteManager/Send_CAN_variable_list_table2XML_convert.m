...
function [obj] = Send_CAN_variable_list_table2XML_convert(obj)
%   1.输入参数：
...
%% 数据有效性检查
% 数据源表格为空时报警并返回
if isempty(obj.sorted_send_CAN_variable_list)
    warning('发送CAN变量清单为空！');
    return;
end
...
% 遍历配置表格
for index = 1 : height(obj.sorted_send_CAN_variable_list)
    
    % 获取变量名对应的port_data节点
    port_data_node = port_data_list_node.getElementsByTagName(...
        obj.sorted_send_CAN_variable_list.('variable_name'){index}).item(0);
    ...
    temp_node = obj.XML_document.createElement(...
        obj.sorted_send_CAN_variable_list.('CAN_message_name'){index});
    direction_tag_node.appendChild(temp_node);

    % 新建当前表格条目各属性对应的节点
    format_node = obj.XML_document.createElement('format');
    if strcmp('Intel', obj.sorted_send_CAN_variable_list.('format'){index})
        format_node.appendChild(obj.XML_document.createTextNode('0'));
    elseif strcmp('Motorola', obj.sorted_send_CAN_variable_list.('format'){index})
    ...
    factor_node.appendChild(obj.XML_document.createTextNode(...
        obj.sorted_send_CAN_variable_list.('factor'){index}));
    offset_node = obj.XML_document.createElement('offset');
    offset_node.appendChild(obj.XML_document.createTextNode(...
        obj.sorted_send_CAN_variable_list.('offset'){index}));
    type_node = obj.XML_document.createElement('type');
    if strcmp('UINT', obj.sorted_send_CAN_variable_list.('type'){index})
        type_node.appendChild(obj.XML_document.createTextNode('0'));
    elseif strcmp('INT', obj.sorted_send_CAN_variable_list.('type'){index})
    ...
    start_bit_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_send_CAN_variable_list.('start_bit')(index)))));
    ...
    bit_length_node.appendChild(obj.XML_document.createTextNode(char(string(...
        obj.sorted_send_CAN_variable_list.('bit_length')(index)))));
