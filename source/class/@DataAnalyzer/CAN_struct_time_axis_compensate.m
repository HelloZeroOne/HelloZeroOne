...
function [obj] = CAN_struct_time_axis_compensate(obj)
%   1.输入参数：
...
%% 遍历所有域名内的信号表格以构建时分秒三列信号，补偿时间轴精度
for index1 = 1 : size(struct_fields, 1)
    ...
    if ~isempty(obj.CAN_signals_struct.(struct_fields{index1}).time_axis)
        ...
        % 按照时间类型的数据进行处理
        absolute_time = datetime(obj.CAN_signals_struct.(...
            struct_fields{index1}).time_axis, 'ConvertFrom', 'posixtime', ...
            'Format', 'HH:mm:ss.SSS');
        [hour, minute, second] = hms(absolute_time);
        if ~isempty(obj.CAN_signals_struct.(struct_fields{index1}).signal_table)
            obj.CAN_signals_struct.(struct_fields{index1}).signal_table.(...
                '时间轴-时') = hour;
            obj.CAN_signals_struct.(struct_fields{index1}).signal_table.(...
                '时间轴-分') = minute;
            obj.CAN_signals_struct.(struct_fields{index1}).signal_table.(...
        ...
        if ~isempty(duplicate_value)
            ...
            % 为数据集时间轴补偿时间精度
            obj.CAN_signals_struct.(struct_fields{index1}).time_axis = ...
                obj.CAN_signals_struct.(struct_fields{index1}).time_axis + ...
                compensation_vector;
